...
class Question(BaseModel):
    ...
    format_re: str | None = None
    answer: str
    tries_count: int = 0
    max_tries: int = -1
    solved: bool = False
    last_wrong_times: list[float] = []  # 记录错误时间戳
    banned_until: float | None = None  # 被禁止答题的截止时间
    ...
    def check_pattern(self, flag: str):
        # No explicit requirement for flag pattern
        if self.format_re is None:
            return True

        check_re = re.compile(self.format_re)
        return check_re.match(flag)
